...
number, x = int(input('Выберите метод сортировки массива(От большего к меньшему(1)/От меньшего к большему(2)/Наугад первый или второй метод(3)/Не сортировать(4)): ')), 0
...
result = [sum([new_matrix[j][i] for i in range(len(new_matrix[0])) if new_matrix[j][i] != 'x⁰']) for j in range(len(new_matrix))]
print('Результат:')
...
#    check = 0
#    for i in range(n):
#        if matrix[j][i] == min(matrix[j]) and check != matrix[j][i]:
#            result_str[i] += matrix[j][i]
#            new_matrix[j].append(result_str[i])
#            check = matrix[j][i]
#        else:
#            new_matrix[j].append(result_str[i])
#result_str[0] = result_str[1] = result_str[2] = 1
print('Расписание:')
for j in range(col):
